    ...
    def egitimKumesiOku(self, dosyaismi, goster=False):
        ...
        with open(dosyaismi,'r') as f:
            for line in f:
                ...
                girdi = tuple([int(i) for i in girdi])
                cikti = tuple([int(i) for i in cikti])
...
if __name__ == '__main__':
    ...
    dst = Destek()
    # Girdi ve çıktılar
    dosya = 'iter_04_01.txt'
    input_adedi, output_adedi, iterasyonlar = dst.egitimKumesiOku(dosya)
    ...
    #yp.hazir_agirlik_tablosu = 'Agirlik_04_03.txt'
    ...
    if yp.hazir_agirlik_tablosu:
        ...
        # Ağırlık tablosu
        yp.w = yp.agirlikTablosuUret(goster=False)
    ...
    state = 1       #state=1 öğren, state=2 sorgu, state=3 ağırlık değişim incele

    if state == 3:
        destek.agirlik_Degisim_incele('_04_03.txt')
